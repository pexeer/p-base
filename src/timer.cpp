...
struct P_CACHELINE_ALIGNMENT TimerControl::Timer {
    ...
    bool operator <(const Timer& rls) {
        return run_time > rls.run_time;
    }
};
...
TimerControl::TimerBucket::TimerBucket() : head_(nullptr),
...
inline void TimerControl::TimerThread::add_timer(Timer* tm) {
    ...
    while (true) {
        ...
        if (min_timestamp_.compare_exchange_weak(min_timestamp, run_time,
                std::memory_order_release, std::memory_order_relaxed)) {
            break;
        }
    }
...
void TimerControl::stop_and_join() {
    timer_thread_.stop_and_join();
}
