...
void LogMessage::set_output_func(OutputFunc output_func) { g_output_func = output_func; }

void LogMessage::set_wf_output_func(OutputFunc output_func) { g_wf_output_func = output_func; }
...
class LogSinkThread {
public:
    LogSinkThread() : log_sinker_(&LogSinkThread::work, this) { current_pos_ = flush_buffer_; }
...
inline void LogBufferManager::add_buffer(int num) {
    ...
    for (int i = 0; i < num; ++i) {
        BufferEntry *node = reinterpret_cast<BufferEntry *>(buffer + i * kLogBufferSize);
...
bool stop_logging() { return p::base::g_log_sinker.stop(); }
