...
public:
    ...
    static T *acquire(uint64_t *id) { return global_group_.add_block(id)->items; }

    static T *find(uint64_t obj_id) { return global_group_.find(obj_id); }
        ...
        ObjectBlock *add_block(uint64_t *block_id) {
            ...
            do {
                ...
                const size_t block_size =
                    group_ptr->block_size.fetch_add(1, std::memory_order_relaxed);
                if (block_size < N) {
...
template <typename T> typename ObjectArena<T>::GlobalGroup ObjectArena<T>::global_group_;
